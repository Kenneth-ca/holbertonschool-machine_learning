...
def inception_network():
    ...
    conv1R = K.layers.Conv2D(64, kernel_size=3, strides=(2, 2), padding="valid",
                             kernel_initializer=init, activation="relu")
    conv1R = conv1R(max_pool0)

    conv1 = K.layers.Conv2D(192, kernel_size=3, strides=(2, 2), padding="same",
                            kernel_initializer=init, activation="relu")
    conv1 = conv1(conv1R)

    conv2 = K.layers.Conv2D(192, kernel_size=7, strides=(2, 2), padding="same",
                            kernel_initializer=init, activation="relu")(conv1)
